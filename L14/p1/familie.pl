...
verisor(X,Y) :-
    tatal(X,A), tatal(Y,B), frate(A,B), barbat(X), X \= Y.
    verisor(X,Y) :-
    mama(X,A), mama(Y,B), sora(A,B), barbat(X), X \= Y.

verisoara(X,Y) :-
    tatal(X,A), tatal(Y,B), frate(A,B), femeie(X), X \= Y.
verisoara(X,Y) :-
    mama(X,A), mama(Y,B), sora(A,B), femeie(X), X \= Y.
